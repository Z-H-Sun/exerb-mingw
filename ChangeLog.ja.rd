...
* 安定版コアをRuby 1.8.7-p160に変更

== 4.8.0

* 安定版コアをRuby 1.8.7-p72に変更

== 4.7.0

* exerb.cppのロード済みライブラリのマークの順序入れ替えを元に戻した
　（Exerbは実行まで行うため、後でマークすると循環参照でループする）
* ライブラリの循環参照に対するテスト（test-nest）を追加
* TODO: 上記修正に伴い、テンポラリにtest-dotとtest-require1は失敗する
* 開発版コアを1.9.0-2に更新
