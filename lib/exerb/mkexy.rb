...
END {
  ...
  if File.exist?(old_recipe_file)
    require 'yaml'
    oldcfg = YAML.load(File.open(old_recipe_file, HAVE_ENCODING ? 'r:utf-8' : 'r')) # YAML needs to process in UTF-8 encoding
  ...
  File.open(recipe_file, HAVE_ENCODING ? 'w:utf-8' : 'w') { |file| # YAML needs to process in UTF-8 encoding
