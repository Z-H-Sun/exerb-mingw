...
module Exerb::Utility2
  ...
  def self.loaded_features(reject_list = [])
    ...
    if RUBY_VERSION >= "1.9.0"
      __REQUIRED_LIBS = ['enc/encdb.so', 'enc/utf_16le.so',
        'enc/trans/transdb.so', 'enc/trans/utf_16_32.so',
        'enc/trans/single_byte.so']
      
      __LOADED_FEATURES.each do |ldf|
        next if __REQUIRED_LIBS.index { |f| ldf.include?(f) } # already in the loaded feature list
        if File.basename(File.dirname(ldf)).downcase == 'enc' and File.extname(ldf).downcase == '.so' # */enc/*.so
          dir_base = File.split(ldf)
          __REQUIRED_LIBS.push('enc/trans/'+dir_base[1]) if File.file?(dir_base[0]+'/trans/'+dir_base[1]) # sometimes, */enc/trans/*.so is sometimes not in the list; need to add it manually if it exists
        end
      end

      __REQUIRED_LIBS.each do |enc|
...
# TODO: Win32API is deprecated after Ruby 1.9.1; also,
